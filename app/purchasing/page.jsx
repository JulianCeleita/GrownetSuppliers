...
import { deletePresentationUrl, purchasingCreate, purchasingUrl, wholesalersUrl } from "../../app/config/urls.config";
...
export const fetchOrderWholesaler = (start, end, token, setOrdersWholesaler, setIsLoading) => {
...
function Purchasing() {
  ...
  const [selectedWholesalers, setSelectedWholesalers] = useState(Array(filteredOrdersWholesaler.length).fill(null));
  ...
  useEffect(() => {
    const filteredOrdersByShort = selectedStatus === "short" ? ordersWholesaler.filter(order => order.short > 0) :
      selectedStatus === "available" ? ordersWholesaler.filter(order => order.short === 0) : ordersWholesaler;

    const filteredOrdersByCategory = selectedCategory === "" ? filteredOrdersByShort :
      filteredOrdersByShort.filter(order => order.category_name === selectedCategory);
